...
function closeSketchLoop(arr: number[][]): number[][] {
  const n = arr.length;
...
function resampleRing(arr: number[][], N: number): number[][] {
  const n = arr.length;
  const seg: number[] = [];
  let per = 0;
  for (let i = 0; i < n; i++) {
...
function detectSketchTail(ring: number[][]): { tip: number[]; body: number[][] } | null {
  const N = ring.length;
...
function smoothSketchRing(ring: number[][], passes = 5): number[][] {
  const N = ring.length;
