...
export async function GET(_req: Request, { params }: Params) {
  ...
  return NextResponse.json({ id: p.id, name: p.name, data: JSON.parse(p.data), role, updatedAt: p.updatedAt.toISOString() });
}
...
export async function PUT(req: Request, { params }: Params) {
  ...
  try {
    ...
    /* optimistic concurrency on a shared book: the client sends the
       updatedAt it loaded; if someone saved since, refuse rather than
       silently overwrite their lettering (last-write-wins lost work) */
    const base = typeof body.baseUpdatedAt === "string" ? new Date(body.baseUpdatedAt) : null;
    if (base && Number.isFinite(base.getTime())) {
      const r = await prisma.project.updateMany({ where: { id, updatedAt: base }, data: patch });
      if (r.count === 0) {
        return NextResponse.json({ error: "Someone else saved this book since you opened it — reload it (or Save a Copy) to avoid overwriting their work." }, { status: 409 });
      }
    } else {
      await prisma.project.update({ where: { id }, data: patch });
    }
    const p = await prisma.project.findUnique({ where: { id }, select: { id: true, name: true, updatedAt: true } });
    return NextResponse.json({ id, name: p?.name ?? body.name, updatedAt: p?.updatedAt.toISOString() ?? null });
  } catch {
